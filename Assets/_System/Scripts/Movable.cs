...
public class Movable : MonoBehaviour
{
    ...
    void VisualizeEllipse(LineRenderer lr)
    {
        ...
        var orbitColor = Random.ColorHSV();
    ...
    float CalcTrueAnomaly()
    {
        ...
        if (Ma >= 0)
        {
            return (float)index / orbitPoints * 360;
        }
        else
        {
            float point = (float)index / orbitPoints * hyperbolaAngleRange * 2;
            return point - hyperbolaAngleRange;
        }
    }
    ...
    Vector3 CalcVelVector()
    {
        float mag = Mathf.Sqrt(Mu * (2 / (transform.position - SOI.transform.position).magnitude - 1 / Ma));
        float newAnomaly = trueAnomaly + .001f * travelDirection;
        if (newAnomaly > 360)
        {
            newAnomaly = .001f;
        }
        else if (newAnomaly < 0)
        {
            newAnomaly = 355.999f;
        }
        Vector3 newPos = GetPos(newAnomaly);
        return (newPos - GetPos(trueAnomaly)).normalized * mag;
    }
    ...
    {
        point = Rotate(new Vector3[] { point }, angle1, focus1, Vector3.up)[0];
        point = Rotate(new Vector3[] { point }, angle2, focus1, Vector3.Cross(new Vector3(focus2.x, 0, focus2.z), Vector3.up))[0];
        point = Rotate(new Vector3[] { point }, angle3, focus1, focus2)[0];
        return point;
    }
}
